---
layout: single
title: "Curriculum Vitae"
permalink: /cv/
author_profile: true
classes: wide
---
{: .text-smaller }
## Experience
---
### **Data Scientist** @ Lloyd's List Intelligence *(2024–Present)*  
- Data science lead in a cross-functional squad, driving delivery from strategy to launch
- Built and deployed production-grade NLP and retrieval pipelines in AWS SageMaker, converting text to embeddings via language models, indexing with FAISS, and applying custom re-ranking, cutting manual screening time by ~50%
- Built and deployed a production-grade entity-resolution pipeline automating ~40% of structured matching cases
- Designed and built a sanctions knowledge graph over public OFAC, UN, and EU data — modelling 25k+ entities in Neo4j, layering semantic search via FAISS (60k vectors), and implementing a two-stage entity resolution pipeline combining cosine similarity with structured attribute scoring (IMO, DOB, call sign) to draw cross-source SAME_AS edges
- Built a LangGraph ReAct agent over the knowledge base with five retrieval tools, enabling multi-hop sanctions queries with full trace logging and a keyword-based evaluation framework
- Prototyped an internal RAG-based product assistant (AWS Bedrock + Streamlit) to accelerate responses to product and data queries for internal users and customers
- Developed proof-of-concept models for vessel fuel consumption and emissions (ML and regression) to support exploratory ESG analytics

### **Senior Data Analyst** @ Lloyd's List Intelligence *(2023–2024)*  
- Delivered statistical and ML models to impute maritime draft, payload, and fuel data, improving data quality for ESG emissions estimates and downstream models
- Collaborated with engineers to build and automate fuzzy matching pipelines in AWS for vessel-port alignment, increasing prediction accuracy and data coverage
- Applied NLP techniques to analyse customer feedback, surfacing data quality gaps that directly guided investment into key core datasets

### **Maritime Data Analyst** @ Lloyd's List Intelligence *(2021–2023)*  
- Validated ML models for ETA, congestion and destination predictions
- Led a team of 3 in developing new analytics product features
- Built real-time dashboards using Tableau and Power BI

## Education
---
**M.Sc. Statistics**, UCL *(2020–2021)*  
*Dissertation:* "Approximate Bayesian Computation for Factor Copula models" — Explored advanced Bayesian techniques for modelling complex dependence structures in finance  
**Projects**: Predictive Modelling (GLMs vs Random Forest vs XGBoost), Risk Management (Copula VaR), ARIMA forecasting

**B.Sc. Mathematics**, AUTh *(2015–2019)*  
*Dissertation:* "Predicting Sleep Quality Using ML" — Developed predictive models to assess sleep quality based on key health indicators  
**Key Modules**: Probability, Time Series, Stochastic Methods in Finance, Information Theory, Regression Models, Deep Learning, Econometrics, Partial Differential Equations

## Skills
---
**Languages**: Python, R, SQL, Fortran, Matlab, SPSS, LaTeX  
**Tools**: Git, Docker, AWS (SageMaker, Bedrock, ECR, Redshift), Neo4j, FAISS, LangGraph, Power BI, Tableau, ElasticSearch

## Certifications
---
- Computational Finance (Financial Engineering & Risk)
- Udemy: Quantum Computing

## Expertise
---
**Statistical & ML**:  
Statistical Modelling, Machine Learning, Deep Learning, Bayesian Inference, Time Series, Feature Engineering, NLP, Knowledge Graphs, Semantic Search, Agentic Systems

**Financial & Quant**:  
Stochastic Calculus, Option Pricing, VaR, Monte Carlo, Copula Modelling, Fixed-Income Analytics, Risk Modelling

## Publications
---
- Bratsas, Charalampos; Koupidis, Kleanthis; Salanova, Josep-Maria; Giannakopoulos, Konstantinos; Kaloudis, Aristeidis; Aifadopoulou, Georgia. 2020. "A Comparison of Machine Learning Methods for the Prediction of Traffic Speed in Urban Places." **Sustainability** 12
